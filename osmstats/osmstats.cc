...
QueryOSMStats::syncUsers( const std::vector<TMUser>& users )
{
    ...
    // For some reason this class uses it's own connection (sdb) instead of the base class's one (db),
    // and it does override connect() without calling the base class implementation, base class 'db' is
    // nullptr and 'worker' cannot be used so we need to create a new one.
    pqxx::work worker( *sdb );
